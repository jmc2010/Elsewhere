-- catalog_search: filter non-destinations here, and return the confirmation
-- count.
--
-- The filter moves off the screen for the same reason the radius and the limit
-- did: a screen deciding what the catalog returns is a product decision living
-- in a component. The shortlist passes the value from config/tuning.ts; the
-- default stays permissive so the flag remains evidence rather than becoming
-- an invisible delete.
--
-- `confirmations` lets the frontier card stop approximating. §5 defines it as
-- "no Google match AND nobody in Elsewhere has reviewed it"; the screen was
-- substituting "unknown to YOU", which is the same thing only while there is
-- exactly one user. The card needs nothing but `> 0`.
