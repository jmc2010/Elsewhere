-- catalog_search: raise the p_limit ceiling, and stop it citing a budget it
-- does not spend.
--
-- 0027 made p_limit a required parameter carrying the catalog pool size from
-- config/tuning.ts, whose starting value is 200. The existing guard rejected
-- it: the ceiling was 100, justified by the Google hydration budget. But
-- catalog_search never calls Google. The two limits were conflated, and the
-- one that actually costs money is enforced in places-proxy.
