-- catalog_search: seed the tiebreak.
--
-- Ordering the pool by `update_time desc, id` was unbiased with respect to
-- distance, which was the goal -- but in a dense area it is not meaningfully
-- ordered by freshness either, because most rows carry the same bulk
-- timestamp. That left `id` deciding membership, and `id` never changes.
--
-- The consequence is not "some places rank lower". It is that 3,782 of
-- downtown Dallas's 3,982 eligible places can never appear, for anybody,
-- ever, no matter how good they are or what the user has told us.
--
-- Seeding it with user_id || current date fixes that and does something the
-- product wants anyway: Dallas shows a different slice on Thursday than on
-- Tuesday, per user. That is the anti-rut principle (§3, and the whole
-- positioning argument against transaction-history competitors) expressed in
-- the query plan rather than built as a feature later.
--
-- Stability matters in both directions: within a day the shortlist must not
-- shuffle under someone halfway through reading it, and across days it must.
