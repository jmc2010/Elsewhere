-- catalog_search returns the caller's most recent lock-in.
--
-- Recency suppression is the only ranking rule that works today -- tag
-- affinity, friend verdicts and confidence weights are all unbuilt. A commit
-- that leaves no trace means tomorrow's shortlist offers tonight's
-- restaurant, which is the rut the product exists to break showing up on its
-- best screen.
--
-- Provisional, because a lock-in is not a visit: review capture resolves it
-- into a verdict or into "did not go".

begin;

drop function if exists catalog_search(
  double precision, double precision, double precision, integer, text,
  text[], text[], uuid, boolean, boolean, boolean, boolean);
...
create or replace function catalog_search(
  ...
  -- or drink. Defaulted true so the flag stays evidence rather than becoming
  -- a silent deletion: a correction flow specifically wants to see suspects.
  -- The shortlist passes false, from config/tuning.ts.
  p_include_non_destinations boolean default true
)
returns table (
  place_id        uuid,
  name            text,
  ...
  -- How many people have recorded any verdict here. Existence evidence, not
  -- an opinion aggregate -- see migration 0039.
...
begin
  ...
  select
    ...
    (select count(*) from visits vi
      where vi.place_id = f.id
        and (p_household_id is null or vi.household_id = p_household_id)),
    -- The CALLER's own verdict. RLS on place_verdicts restricts this to
    -- auth.uid() already, so no user parameter is needed and none is offered
    -- -- a function that took one could be asked for somebody else's history.
    (select v.verdict from place_verdicts v where v.place_id = f.id),
    freshness_band(f.update_time),
    -- The caller's most recent lock-in here. RLS scopes place_lockins to
    -- auth.uid(), so this is theirs and nobody else's. Drives provisional
    -- recency suppression: without it the app offers tonight's restaurant
    -- again tomorrow morning.
    (select max(l.locked_at) from place_lockins l where l.place_id = f.id),
    f.elsewhere_confirmations
  from filtered f
