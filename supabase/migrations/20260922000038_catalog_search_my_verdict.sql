-- catalog_search returns the caller's own verdict, and the freshness band.
--
-- Without the verdict the shortlist has no Layer 3 at all: it cannot mark a
-- place as one you already know, cannot apply a cooldown, and cannot tell
-- whether a rural town is exhausted -- "You've been to all four" (§10) needs
-- to know how many of the four you have been to.
--
-- The band is returned as well as the raw timestamp, so the card treatment
-- (§5) and the pool ordering read the same boundaries from one definition
-- rather than two copies that drift.
--
-- No user parameter. RLS scopes place_verdicts to auth.uid(), so the caller
-- can only ever see their own -- and a function that accepted a user id could
-- be asked for somebody else's history.
